...
let bifurcation: IXnPlusOne = function (r, x) {
    return r * x * (1.0 - x);
}

let r: number = 3, x: number = 0.5, idx: number = 0;
...
let bifurcWithCnt: IXnPlusOneWithCounter = <IXnPlusOneWithCounter>function (r, x) {
...
let r: number = 3.1, x: number = 0.5, idx: number = 0;
for (idx = 0; idx < 10; idx += 1) {
    x = bifurcWithCnt(r, x);
